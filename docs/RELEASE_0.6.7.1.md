# Battle Axe Design Studio v0.6.7.1
## Command, ACW AI & Scenario Portability

### Release intent

v0.6.7.1 is a focused Scenario Builder / command / playtest-engine release built from the v0.6.5.0 battlefield-authoring baseline. It does not redesign or refactor the working PowerPoint/SVG battlefield compiler.
...
The v0.6.5.0 battlefield/compiler stack remains the protected baseline for this release. Functional logic in the structured compiler, detector, battlefield crop/state, Geometry Explorer, Terrain Review, map view/scale, and road-movement modules is preserved. Release cache/version query references may differ as required for v0.6.7.1 deployment.

## v0.6.7.1 legacy-import safety hotfix

Legacy scenario-only JSON created by older Studio releases may not contain a map at all. v0.6.7.1 detects that condition before overwrite. If a current battlefield is loaded, Studio offers to retain its authoritative map/approved terrain while importing the legacy scenario. A pre-import recovery copy is also saved automatically and can be restored with **Restore Previous Project**. This cannot reconstruct map or deployment data that was never present in an old JSON after the previous browser state has already been overwritten.
