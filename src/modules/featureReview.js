import { highlightFeature, clearOverlay } from './mapView.js?v=0.6.9.1';
...
export function setupFeatureReview(state,persist,svg){
  ...
  function openManualEditor(){const dialog=document.querySelector('#manualFeatureDialog');document.querySelector('#manualFeatureName').value=`Manual feature ${(state.project.manualFeatures||[]).length+1}`;document.querySelector('#manualFeatureType').value='polygon';dialog?.showModal();document.querySelector('#manualFeatureName')?.select();}
  function beginManual(){const name=document.querySelector('#manualFeatureName')?.value.trim(),type=document.querySelector('#manualFeatureType')?.value;if(!name){document.querySelector('#manualFeatureName')?.focus();return;}if(!['polygon','line','point'].includes(type))return;document.querySelector('#manualFeatureDialog')?.close();manualDraw={type,name,points:[]};finishManual.hidden=type==='point';cancelManual.hidden=false;mapHost.classList.add('manual-drawing');mapHint.textContent=type==='point'?'Click the missing feature location.':'Click vertices around/along the missing feature, then choose Finish drawing.';clearOverlay(overlay);}
  function completeManual(){if(!manualDraw)return;const min=manualDraw.type==='polygon'?3:manualDraw.type==='line'?2:1;if(manualDraw.points.length<min){mapHint.textContent=`Add at least ${min} point${min>1?'s':''}, or Cancel.`;return;}const name=manualDraw.name,id=`manual-${Date.now().toString(36)}`;const f={id,name,category:'Manually added features',proposal:'Manual source geometry',cls:'Unknown',effects:[],reason:'Manually drawn by the designer because the structured compiler did not detect this battlefield feature.',detectionConfidence:100,interpretationConfidence:100,provenance:'Manual Studio geometry',geometry:{type:manualDraw.type,parts:[{closed:manualDraw.type==='polygon',points:manualDraw.points}]}};state.project.manualFeatures=state.project.manualFeatures||[];state.project.manualFeatures.push(f);persist();stopManual();renderRows(false);select(id);}
  ...
  document.querySelector('#addMissingFeature')?.addEventListener('click',openManualEditor);document.querySelector('#beginManualFeature')?.addEventListener('click',beginManual);finishManual?.addEventListener('click',completeManual);cancelManual?.addEventListener('click',stopManual);
