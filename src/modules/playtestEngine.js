import { getEffectiveRuleset } from '../rules/ruleset.js?v=0.5.0.2-phase3';
...
function activate(u,ctx,unalerted){
  ...
  const firstAction=chooseTacticalAction(legalActionsForUnit(u,ctx));ctx.event('ai_action_choice',u,{action:firstAction?.type||null,target:firstAction?.target?.id||null,targetName:firstAction?.target?.name||null,legalActionCount:legalActionsForUnit(u,ctx).length});const first=executeAction(firstAction,u,ctx);if(!first||u.destroyed||['charge','charge_commander'].includes(first))return;if(ctx.units.some(t=>!t.destroyed&&coalition(t.faction)!==coalition(u.faction)&&countsAsEngaged(u,t)))return;if(commandTest(u,ctx)){const secondAction=chooseTacticalAction(legalActionsForUnit(u,ctx));ctx.event('ai_action_choice',u,{action:secondAction?.type||null,target:secondAction?.target?.id||null,targetName:secondAction?.target?.name||null,legalActionCount:legalActionsForUnit(u,ctx).length,secondAction:true});executeAction(secondAction,u,ctx);}
}
...
export function buildRuntimeFromStudio(state,settings={}){const s=state.project.scenario,rules=getEffectiveRuleset(s),profileMap=new Map(rules.unitLibrary.map(x=>[x.profile,x])),units=[],commanders=[],scale=Number(settings.measurementScale||2);for(const [faction,cmds] of Object.entries(s.commands||{})){for(let ci=0;ci<(cmds||[]).length;ci++){const c=cmds[ci];const cp=s.deployment?.commanderPlacements?.[c.id];if(c.commander&&cp){const p=pctToTable(cp,state);commanders.push({id:`cmd-${c.id}`,kind:'commander',commandId:c.id,name:c.commander,faction,x:p.x,y:p.y,baseMm:Number(rules.core.baseGeometry.commanderBaseMm||25),traits:[],destroyed:false,isGeneral:!!c.isGeneral||ci===0});}for(const u of c.units||[]){const p0=s.deployment?.placements?.[u.id];if(!p0)continue;const p=pctToTable(p0,state),base=profileMap.get(u.profile)||{m:2,c:2,a:4,pts:1,traits:[]};units.push({id:u.id,name:u.name,profile:u.profile,faction,commandId:c.id,x:p.x,y:p.y,facing:Number(p0.facing??(faction==='French'?0:180)),move:base.m,combat:base.c,armor:base.a,points:base.pts,baseMm:Number(base.baseMm||rules.core.baseGeometry.unitBaseMm||50),traits:[...new Set([...(base.traits||[]),...(u.traits||[]), ...(rules.capabilities.italianWarsTraits&&(u.profile==='Spanish Tercio'||(base.traits||[]).includes('Tercio'))?['Arquebus','Fury','Elite','Pikes']:[]), ...(rules.capabilities.campBaggageAssets&&['Camp','Baggage Train'].includes(u.profile)?['Immobile','Army Asset',u.profile]:[])])],damage:0,destroyed:false,inactive:false});}}}
