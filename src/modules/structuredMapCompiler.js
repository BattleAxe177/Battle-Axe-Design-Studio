...
export function terrainContextHints(text){
  const q=clean(text);if(!q)return[];const out=[];
  for(const c of TERRAIN_ONTOLOGY){if(c.sourceType==='Battlefield Boundary'||c.sourceType==='Decorative')continue;for(const pattern of c.patterns){pattern.lastIndex=0;if(pattern.test(q)){out.push({family:c.family,sourceType:c.sourceType,cls:c.cls,effects:[...c.effects]});break;}}}
  return out.filter((x,i,a)=>a.findIndex(y=>y.sourceType===x.sourceType)===i);
}
export function contextTerrainEvidence(mapNotes='',historicalContext=''){
  return {mapNotes:terrainContextHints(mapNotes),historical:terrainContextHints(historicalContext)};
}
function contextMatch(sem,evidence){
  const inNotes=evidence.mapNotes.some(x=>x.sourceType===sem.sourceType||x.cls===sem.cls||x.family===sem.family);
  const inHistory=evidence.historical.some(x=>x.sourceType===sem.sourceType||x.cls===sem.cls||x.family===sem.family);
  return {inNotes,inHistory};
}
...
function likelyBoundary(shapes,slide,playSpace){const desired=(Number(playSpace?.width)||0)/(Number(playSpace?.height)||1),slideArea=Math.max(1,rectArea(slide)),strong=[],weak=[];for(const s of shapes){const q=clean(s.descr||s.name),sem=classifyTerrainDescription(s.descr||s.name);if(sem.sourceType!=='Battlefield Boundary')continue;if(/battlefield boundary|table boundary|play ?area|map boundary/.test(q))strong.push(s);else weak.push(s);}const pick=(list,respectAspect=true)=>list.map(s=>{const area=rectArea(s.rect)/slideArea,ap=desired>0?Math.abs(Math.log(Math.max(.01,aspect(s.rect)/desired))):0;return{s,area,ap,score:(respectAspect?ap*5:0)-Math.min(.8,area)}}).filter(x=>x.area>.04&&x.area<.98&&(!respectAspect||desired<=0||x.ap<.5)).sort((a,b)=>a.score-b.score)[0]?.s.rect;if(strong.length){const b=pick(strong,false);if(b)return b;}if(weak.length){const aspectMatch=pick(weak,true);if(aspectMatch)return aspectMatch;const authored=pick(weak,false);if(authored)return authored;}const candidates=shapes.filter(s=>s.closed&&s.nodeType==='auto-shape').map(s=>{const area=rectArea(s.rect)/slideArea,ap=desired>0?Math.abs(Math.log(Math.max(.01,aspect(s.rect)/desired))):0;return{s,area,ap,score:ap*4-Math.min(.8,area)}}).filter(x=>x.area>.05&&x.area<.95&&(desired<=0||x.ap<.42));return candidates.sort((a,b)=>a.score-b.score)[0]?.s.rect||slide;}
...
export async function compilePptxTerrain(file,{playSpace=null,mapNotes='',historicalContext=''}={}){
  const z=await unzipSelected(file,['ppt/slides/slide1.xml','ppt/presentation.xml']);const slideText=z['ppt/slides/slide1.xml'];if(!slideText)throw new Error('PPTX slide1.xml was not found.');const slideDoc=parseXml(slideText,'PPTX slide');const slide=slideSize(z['ppt/presentation.xml']);const tree=descendants(slideDoc,'spTree')[0];if(!tree)throw new Error('PPTX slide did not contain a shape tree.');const shapes=collectShapes(tree);const boundary=likelyBoundary(shapes,slide,playSpace),features=[],candidates=[],counts=new Map(),contextEvidence=contextTerrainEvidence(mapNotes,historicalContext);
  for(const s of shapes){if(!meaningfulShape(s,boundary))continue;const source=preferredSourceText(s),shapeAspect=Math.max(aspect(s.rect),1/Math.max(.001,aspect(s.rect))),sem=classifyTerrainDescription(source,{shapeKind:s.nodeType,aspect:shapeAspect,closed:s.closed});
    ...
    if(sem.matched){const key=sem.sourceType,n=(counts.get(key)||0)+1;counts.set(key,n);const ctx=contextMatch(sem,contextEvidence),confidence=Math.min(99,sem.confidence+(ctx.inNotes?4:0)+(ctx.inHistory?2:0));const contextText=[ctx.inNotes?'your Input Map Notes':null,ctx.inHistory?'the Historical Battlefield Description':null].filter(Boolean);const reason=`PowerPoint-authored geometry identified from source metadata “${identity}”. Classified through the general terrain ontology (${sem.family}).${contextText.length?` This interpretation is also supported by ${contextText.join(' and ')}.`:''}`;const rec={id:`pptx-${s.id||features.length+candidates.length+1}`,name:featureName(sem.sourceType,source,n),sourceLabel:source||identity,sourceType:sem.sourceType,category:sem.family,proposal:sem.sourceType,cls:sem.cls,effects:sem.effects,detectionConfidence:100,interpretationConfidence:confidence,confidence,box,geometry,elementIds:[],reason,provenance:'PPTX authored geometry',contextSupport:{mapNotes:ctx.inNotes,historical:ctx.inHistory}};if(confidence>=76)features.push(rec);else candidates.push({...rec,kind:sem.sourceType});
    }else if(source||s.nodeType==='connector'||(s.nodeType==='auto-shape'&&s.parts.some(p=>p.points.length>4))){candidates.push({id:`pptx-unknown-${s.id||candidates.length+1}`,name:source||`Unclassified authored shape ${s.id||candidates.length+1}`,sourceLabel:source||identity,sourceType:'Unknown',kind:'unclassified PowerPoint-authored terrain geometry',category:'Unclassified source geometry',proposal:'Review source-authored geometry',cls:'Unknown',effects:[],detectionConfidence:100,interpretationConfidence:0,confidence:0,box,geometry,elementIds:[],reason:`The PowerPoint contains explicit authored geometry inside the battlefield, but its wording is not in the terrain ontology. It is retained for human classification instead of being discarded.${[...contextEvidence.mapNotes,...contextEvidence.historical].length?' Scenario context was analyzed, but it was not specific enough to assign this shape safely.':''}`,provenance:'PPTX authored geometry'});}
  }
  const byType={};for(const f of features)byType[f.sourceType]=(byType[f.sourceType]||0)+1;return{features,candidates,boundary,slideBounds:slide,shapes,descriptions:shapes.map(s=>s.descr).filter(Boolean),contextEvidence,stats:{structured:true,source:'pptx',shapes:shapes.length,promoted:features.length,explorer:candidates.length,contextHints:[...contextEvidence.mapNotes,...contextEvidence.historical].map(x=>x.sourceType),counts:byType,summary:Object.entries(byType).map(([k,v])=>`${k} ${v}`).join(' · ')}};
}

function svgRootBounds(svg){
  const vb=svg?.viewBox?.baseVal;if(vb?.width&&vb?.height)return{x:vb.x,y:vb.y,w:vb.width,h:vb.height};
  const raw=(svg?.getAttribute?.('viewBox')||'').trim().split(/[ ,]+/).map(Number);if(raw.length===4&&raw.every(Number.isFinite)&&raw[2]>0&&raw[3]>0)return{x:raw[0],y:raw[1],w:raw[2],h:raw[3]};
  const num=v=>{const n=parseFloat(String(v||''));return Number.isFinite(n)?n:0},w=num(svg?.getAttribute?.('width')),h=num(svg?.getAttribute?.('height'));return{x:0,y:0,w:w||1280,h:h||720};
}
export function registerPptxBoundaryToSvg(compiled,svg){
  const b=compiled?.boundary,slide=compiled?.slideBounds,target=svgRootBounds(svg);if(!b||!slide?.w||!slide?.h||!target.w||!target.h)return null;
  const x=target.x+((b.x-slide.x)/slide.w)*target.w,y=target.y+((b.y-slide.y)/slide.h)*target.h,width=(b.w/slide.w)*target.w,height=(b.h/slide.h)*target.h;
  if(![x,y,width,height].every(Number.isFinite)||width<=0||height<=0)return null;return{x,y,width,height,source:'pptx-registration'};
}
