...
export const TERRAIN_ONTOLOGY=[
  ...
  {family:'Routes',sourceType:'Road',cls:'Road',effects:['Road'],confidence:93,patterns:[/\broad(s)?\b/,/\bavenue(s)?\b/,/\bcauseway(s)?\b/,/\bturnpike(s)?\b/]},
  ...
  {family:'Routes',sourceType:'Sunken Road',cls:'Road',effects:['Road'],confidence:94,patterns:[/\bsunken road\b/,/\bhollow way\b/,/\bsunken lane\b/]},
...
export function classifyTerrainDescription(description,{shapeKind='shape',aspect=1,closed=true}={}){
  ...
  // Source-authored explicit no-effect language overrides default gameplay effects without erasing the semantic terrain class.
  if(/\b(?:no game (?:impact|effect)|no gameplay (?:impact|effect)|no effect)\b/.test(q))effects=[];
...
function applyTransform(t,p){return{x:t.a*p.x+t.c*p.y+t.e,y:t.b*p.x+t.d*p.y+t.f}}
function multiplyTransform(a,b){return{a:a.a*b.a+a.c*b.b,b:a.b*b.a+a.d*b.b,c:a.a*b.c+a.c*b.d,d:a.b*b.c+a.d*b.d,e:a.a*b.e+a.c*b.f+a.e,f:a.b*b.e+a.d*b.f+a.f}}
function translateTransform(x,y){return{a:1,b:0,c:0,d:1,e:x,f:y}}
function scaleTransform(x,y){return{a:x,b:0,c:0,d:y,e:0,f:0}}
function rotateTransform(deg){const r=deg*Math.PI/180,c=Math.cos(r),s=Math.sin(r);return{a:c,b:s,c:-s,d:c,e:0,f:0}}
function aroundPointTransform(cx,cy,inner){return multiplyTransform(translateTransform(cx,cy),multiplyTransform(inner,translateTransform(-cx,-cy)))}
function boolAttr(el,name){const v=String(el?.getAttribute(name)||'').toLowerCase();return v==='1'||v==='true'}
function drawingRotation(el){const x=xfrm(el),raw=Number(x?.getAttribute('rot')||0);return Number.isFinite(raw)?raw/60000:0}
function xfrmOrientation(x,rect){if(!x||!rect)return identityTransform();const flipH=boolAttr(x,'flipH'),flipV=boolAttr(x,'flipV'),rot=Number(x.getAttribute('rot')||0)/60000,cx=rect.x+rect.w/2,cy=rect.y+rect.h/2;let inner=scaleTransform(flipH?-1:1,flipV?-1:1);if(rot)inner=multiplyTransform(rotateTransform(rot),inner);return aroundPointTransform(cx,cy,inner)}
function composeGroup(parent,grp){const gx=xfrm(grp);if(!gx)return parent;const off=pointNode(gx,'off')||{x:0,y:0},ext=child(gx,'ext'),chOff=pointNode(gx,'chOff')||{x:0,y:0},chExt=child(gx,'chExt');if(!ext||!chExt)return parent;const w=Number(ext.getAttribute('cx')||1),h=Number(ext.getAttribute('cy')||1),sx=w/Math.max(1,Number(chExt.getAttribute('cx')||1)),sy=h/Math.max(1,Number(chExt.getAttribute('cy')||1));const base=multiplyTransform(translateTransform(off.x,off.y),multiplyTransform(scaleTransform(sx,sy),translateTransform(-chOff.x,-chOff.y)));const orient=xfrmOrientation(gx,{x:off.x,y:off.y,w,h});return multiplyTransform(parent,multiplyTransform(orient,base));}
function rectBounds(points){if(!points?.length)return null;const xs=points.map(p=>p.x),ys=points.map(p=>p.y);const x=Math.min(...xs),y=Math.min(...ys),x2=Math.max(...xs),y2=Math.max(...ys);return{x,y,w:x2-x,h:y2-y};}
function shapeMapper(el,parent){const r=rectFromXfrm(el);if(!r)return null;const own=xfrmOrientation(xfrm(el),r),full=multiplyTransform(parent,own);return{raw:r,map:p=>applyTransform(full,p),full};}
function localRect(el,parent){const sm=shapeMapper(el,parent);if(!sm)return null;const r=sm.raw;return rectBounds([{x:r.x,y:r.y},{x:r.x+r.w,y:r.y},{x:r.x+r.w,y:r.y+r.h},{x:r.x,y:r.y+r.h}].map(sm.map));}
function shapeTransformRect(el,parent){const sm=shapeMapper(el,parent);if(!sm)return null;return{raw:sm.raw,slide:localRect(el,parent),map:sm.map};}
...
function pathParts(el,parent){const sr=shapeTransformRect(el,parent);if(!sr)return[];const cust=firstDesc(el,'custGeom'),prst=firstDesc(el,'prstGeom');const corner=p=>sr.map(p);if(!cust){const r=sr.raw;if(lname(el)==='cxnSp'||prst?.getAttribute('prst')==='line')return[{closed:false,points:[corner({x:r.x,y:r.y}),corner({x:r.x+r.w,y:r.y+r.h})]}];return[{closed:true,points:[corner({x:r.x,y:r.y}),corner({x:r.x+r.w,y:r.y}),corner({x:r.x+r.w,y:r.y+r.h}),corner({x:r.x,y:r.y+r.h})]}];}
  const out=[];for(const path of descendants(cust,'path')){const pw=Number(path.getAttribute('w')||sr.raw.w||1),ph=Number(path.getAttribute('h')||sr.raw.h||1);let points=[],current=null,closed=false;const map=p=>sr.map({x:sr.raw.x+(Number(p.x)||0)/Math.max(1,pw)*sr.raw.w,y:sr.raw.y+(Number(p.y)||0)/Math.max(1,ph)*sr.raw.h});for(const cmd of [...path.children]){const n=lname(cmd),pts=descendants(cmd,'pt').map(p=>({x:Number(p.getAttribute('x')||0),y:Number(p.getAttribute('y')||0)}));if(n==='moveTo'&&pts[0]){if(points.length)out.push({closed,points});points=[];current=map(pts[0]);points.push(current);closed=false;}else if(n==='lnTo'&&pts[0]){current=map(pts[0]);points.push(current);}else if(n==='cubicBezTo'&&pts.length>=3&&current){const [c1,c2,e]=pts.map(map);const samples=sampleCubic(current,c1,c2,e,4);points.push(...samples);current=e;}else if(n==='quadBezTo'&&pts.length>=2&&current){const c1=map(pts[0]),e=map(pts[1]),p1={x:current.x+2/3*(c1.x-current.x),y:current.y+2/3*(c1.y-current.y)},p2={x:e.x+2/3*(c1.x-e.x),y:e.y+2/3*(c1.y-e.y)};points.push(...sampleCubic(current,p1,p2,e,4));current=e;}else if(n==='close')closed=true;}if(points.length)out.push({closed,points});}
  return out.length?out:[{closed:true,points:[corner({x:sr.raw.x,y:sr.raw.y}),corner({x:sr.raw.x+sr.raw.w,y:sr.raw.y}),corner({x:sr.raw.x+sr.raw.w,y:sr.raw.y+sr.raw.h}),corner({x:sr.raw.x,y:sr.raw.y+sr.raw.h})]}];
}
function directChild(el,n){return [...(el?.children||[])].find(x=>lname(x)===n)||null}
function colorToken(container){if(!container)return'';for(const x of descendants(container,'srgbClr')){const v=x.getAttribute('val');if(v)return`#${v.toUpperCase()}`;}for(const x of descendants(container,'schemeClr')){const v=x.getAttribute('val');if(v)return`scheme:${v}`;}return'';}
function shapeStyle(el){const sp=directChild(el,'spPr');if(!sp)return{fill:'',stroke:'',fillKind:'none'};let fill='',fillKind='none';for(const x of [...sp.children]){const n=lname(x);if(['solidFill','gradFill','pattFill','blipFill','noFill'].includes(n)){fillKind=n;fill=colorToken(x);break;}}const ln=directChild(sp,'ln'),stroke=colorToken(ln);return{fill,stroke,fillKind};}
function collectShapes(container,parent=identityTransform(),out=[],inherited=[]){for(const el of [...container.children]){if(lname(el)==='grpSp'){const meta=shapeMeta(el),groupSource=preferredSourceText(meta),next=groupSource?[...inherited,groupSource]:inherited;const t=composeGroup(parent,el);collectShapes(el,t,out,next);continue;}if(!['sp','cxnSp','pic','graphicFrame'].includes(lname(el)))continue;const meta=shapeMeta(el),rect=localRect(el,parent);if(!rect?.w&&!rect?.h)continue;const parts=pathParts(el,parent),ownSource=preferredSourceText(meta),inheritedSource=ownSource?'':(inherited.at(-1)||''),style=shapeStyle(el);out.push({...meta,inheritedSource,ancestorSources:[...inherited],nodeType:nodeType(el),rect,parts,closed:parts.some(p=>p.closed),style,rotation:drawingRotation(el),partCount:parts.length,pointCount:parts.reduce((n,p)=>n+p.points.length,0)});}return out;}

// Pure affine helper exported for regression tests and future compiler diagnostics.
export function transformPointForDrawingMl(point,{off={x:0,y:0},ext={w:1,h:1},rotation=0,flipH=false,flipV=false}={}){const rect={x:Number(off.x)||0,y:Number(off.y)||0,w:Number(ext.w)||1,h:Number(ext.h)||1};let inner=scaleTransform(flipH?-1:1,flipV?-1:1);if(rotation)inner=multiplyTransform(rotateTransform(rotation),inner);return applyTransform(aroundPointTransform(rect.x+rect.w/2,rect.y+rect.h/2,inner),point);}
...
function preferredSourceText(s){const d=String(s.descr||'').trim();if(d)return d;const n=String(s.name||'').trim();if(n&&!/^(rectangle|freeform|shape|picture|text ?box|textbox|group|connector|straight connector|line|arc|oval|ellipse|auto ?shape)\b/i.test(n))return n;return String(s.inheritedSource||'').trim();}
...
function inferTerrainFromShape(s){const fill=String(s.style?.fill||'').toUpperCase(),stroke=String(s.style?.stroke||'').toUpperCase(),shapeAspect=Math.max(aspect(s.rect),1/Math.max(.001,aspect(s.rect)));
  // Glendale-style woods are a single PowerPoint freeform containing many disconnected closed subpaths.
  // When author metadata is absent, the green authored fill + complex multipart geometry is strong visual evidence.
  if(fill==='#3B7D23'&&s.closed&&(s.partCount>=3||s.pointCount>=40))return{matched:true,family:'Vegetation',sourceType:'Dense Woodland',cls:'Dense Wood',effects:['Difficult','Obscuring'],confidence:78,source:'visual style inference'};
  if(stroke==='#61A6D0'&&!s.style?.fill&&shapeAspect>1.4)return{matched:true,family:'Hydrology',sourceType:'Watercourse',cls:'Stream',effects:['Difficult'],confidence:66,source:'visual style inference'};
  if(stroke==='#996600'&&!s.style?.fill&&shapeAspect>1.3)return{matched:true,family:'Walls & barriers',sourceType:'Fence / Palisade',cls:'Fence',effects:[],confidence:64,source:'visual style inference'};
  if(stroke==='#D7CDA1'&&shapeAspect>2.4)return{matched:true,family:'Routes',sourceType:'Road',cls:'Road',effects:['Road'],confidence:58,source:'visual style inference'};
  return null;
}
...
export async function compilePptxTerrain(file,{playSpace=null}={}){
  ...
  for(const s of shapes){if(!meaningfulShape(s,boundary))continue;const ownSource=preferredSourceText({...s,inheritedSource:''}),source=preferredSourceText(s),shapeAspect=Math.max(aspect(s.rect),1/Math.max(.001,aspect(s.rect)));let sem=classifyTerrainDescription(source,{shapeKind:s.nodeType,aspect:shapeAspect,closed:s.closed}),basis=ownSource?'object metadata':s.inheritedSource?'group metadata':'visual geometry/style';
    if(!sem.matched){const inferred=inferTerrainFromShape(s);if(inferred)sem=inferred;}
    if(sem.sourceType==='Battlefield Boundary'||sem.sourceType==='Decorative'||/\bbackground\b/i.test(source))continue;
    const box=clippedBoxPct(s.rect,boundary);if(!box)continue;const geometry=geometryPct(s.parts,boundary),identity=source||s.name||`PowerPoint shape ${s.id||features.length+candidates.length+1}`;
    let conflict='';if(ownSource&&s.inheritedSource){const ownSem=classifyTerrainDescription(ownSource,{shapeKind:s.nodeType,aspect:shapeAspect,closed:s.closed}),parentSem=classifyTerrainDescription(s.inheritedSource,{shapeKind:s.nodeType,aspect:shapeAspect,closed:s.closed});if(ownSem.matched&&parentSem.matched&&ownSem.cls!==parentSem.cls)conflict=` Child metadata “${ownSource}” overrides parent-group metadata “${s.inheritedSource}”.`;}
    const rotationNote=Math.abs(s.rotation||0)>.001?` PowerPoint rotation ${Number(s.rotation).toFixed(2)}° was applied to the authored path before battlefield normalization.`:'';
    if(sem.matched){const key=sem.sourceType,n=(counts.get(key)||0)+1;counts.set(key,n);const inheritedNote=!ownSource&&s.inheritedSource?` Semantic metadata was inherited from the enclosing PowerPoint group “${s.inheritedSource}”.`:'';const inferredNote=!source?` No terrain alt text was present; classification was inferred from authored geometry/style and should remain reviewable.`:'';const reason=`PowerPoint-authored geometry identified from ${basis}${source?` “${identity}”`:''}. Classified through the general terrain ontology (${sem.family}).${inheritedNote}${inferredNote}${conflict}${rotationNote}`;const rec={id:`pptx-${s.id||features.length+candidates.length+1}`,name:featureName(sem.sourceType,source,n),sourceLabel:source||identity,sourceType:sem.sourceType,category:sem.family,proposal:sem.sourceType,cls:sem.cls,effects:sem.effects,detectionConfidence:100,interpretationConfidence:sem.confidence,confidence:sem.confidence,box,geometry,elementIds:[],reason,provenance:'PPTX authored geometry',metadataSource:basis,groupMetadata:s.inheritedSource||'',rotationApplied:Number(s.rotation||0)};if(sem.confidence>=76)features.push(rec);else candidates.push({...rec,kind:sem.sourceType});
    }else if(source||s.nodeType==='connector'||(s.nodeType==='auto-shape'&&s.parts.some(p=>p.points.length>4))){candidates.push({id:`pptx-unknown-${s.id||candidates.length+1}`,name:source||`Unclassified authored shape ${s.id||candidates.length+1}`,sourceLabel:source||identity,sourceType:'Unknown',kind:'unclassified PowerPoint-authored terrain geometry',category:'Unclassified source geometry',proposal:'Review source-authored geometry',cls:'Unknown',effects:[],detectionConfidence:100,interpretationConfidence:0,confidence:0,box,geometry,elementIds:[],reason:`The PowerPoint contains explicit authored geometry inside the battlefield, but its wording/style is not in the terrain ontology. It is retained for human classification instead of being discarded.${rotationNote}`,provenance:'PPTX authored geometry',metadataSource:basis,groupMetadata:s.inheritedSource||'',rotationApplied:Number(s.rotation||0)});}
  }
