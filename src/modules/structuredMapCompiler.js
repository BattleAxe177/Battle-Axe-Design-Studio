...
function translate(tx,ty){return{a:1,b:0,c:0,d:1,e:tx,f:ty}} function scale(sx,sy){return{a:sx,b:0,c:0,d:sy,e:0,f:0}} function rotate(rad){const c=Math.cos(rad),s=Math.sin(rad);return{a:c,b:s,c:-s,d:c,e:0,f:0}}
function xfrmMatrix(x,rect){if(!x||!rect)return identityTransform();const rot=(Number(x.getAttribute('rot')||0)/60000)*Math.PI/180,flipH=x.getAttribute('flipH')==='1'||x.getAttribute('flipH')==='true',flipV=x.getAttribute('flipV')==='1'||x.getAttribute('flipV')==='true',cx=rect.x+rect.w/2,cy=rect.y+rect.h/2;let m=identityTransform();m=multiplyTransform(translate(cx,cy),m);if(rot)m=multiplyTransform(rotate(rot),m);m=multiplyTransform(scale(flipH?-1:1,flipV?-1:1),m);m=multiplyTransform(translate(-cx,-cy),m);return m;}
function composeGroup(parent,grp){const gx=xfrm(grp);if(!gx)return parent;const off=pointNode(gx,'off')||{x:0,y:0},ext=child(gx,'ext'),chOff=pointNode(gx,'chOff')||{x:0,y:0},chExt=child(gx,'chExt');if(!ext||!chExt)return parent;const w=Number(ext.getAttribute('cx')||1),h=Number(ext.getAttribute('cy')||1),sx=w/Math.max(1,Number(chExt.getAttribute('cx')||1)),sy=h/Math.max(1,Number(chExt.getAttribute('cy')||1));let local=multiplyTransform(translate(off.x,off.y),multiplyTransform(scale(sx,sy),translate(-chOff.x,-chOff.y)));local=multiplyTransform(xfrmMatrix(gx,{x:off.x,y:off.y,w,h}),local);return multiplyTransform(parent,local);}
function shapeMatrix(el,parent){const r=rectFromXfrm(el);if(!r)return parent;return multiplyTransform(parent,xfrmMatrix(xfrm(el),r));}
function bboxOfPoints(points){if(!points.length)return null;const xs=points.map(p=>p.x),ys=points.map(p=>p.y);return{x:Math.min(...xs),y:Math.min(...ys),w:Math.max(...xs)-Math.min(...xs),h:Math.max(...ys)-Math.min(...ys)};}
function localRect(el,parent){const r=rectFromXfrm(el);if(!r)return null;const m=shapeMatrix(el,parent),pts=[{x:r.x,y:r.y},{x:r.x+r.w,y:r.y},{x:r.x+r.w,y:r.y+r.h},{x:r.x,y:r.y+r.h}].map(p=>applyTransform(m,p));return bboxOfPoints(pts);}
function shapeTransformRect(el,parent){const r=rectFromXfrm(el);if(!r)return null;return{raw:r,slide:localRect(el,parent),matrix:shapeMatrix(el,parent)};}
...
function preferredSourceText(s){const d=String(s.descr||'').trim();if(d)return d;const inherited=String(s.inheritedDescr||'').trim();if(inherited)return inherited;const n=String(s.name||'').trim();if(n&&!/^(rectangle|freeform|shape|picture|text ?box|textbox|group|connector|straight connector|line|arc|oval|ellipse|auto ?shape)\b/i.test(n))return n;return'';}
