...
function actionFromText(t,{security=false}={}){
  ...
  if(/\b(defend|remain|stay)\b/.test(t))return'Defend';
  return null;
}
...
function splitConditionClauses(text){
  ...
  const lead=/(?:until|while|when|once|after|release\s+(?:once|when|after)?|commit\s+(?:once|when|after)?|not\s+before)\s+(.+?)(?=\bthen\b|[.;]|$)/i.exec(lower);
  return lead?.[1]?.trim()||'';
}
...
function parseConditionTree(raw,context){
  const clause=splitConditionClauses(raw);if(!clause)return{condition:null,guardCondition:null,unresolved:[]};
  ...
  for(const piece of pieces){const positive=piece.replace(/^\s*(?:no|not)\s+/i,'').replace(/\bhas\s+not\b/i,'has').replace(/\bhave\s+not\b/i,'have'),p=simplePredicate(positive,context);if(p)predicates.push(p);else if(piece.trim())unresolved.push(piece.trim());}
  if(!predicates.length)return{condition:null,guardCondition:null,unresolved:unresolved.length?unresolved:[clause]};
  if(unresolved.length)return{condition:null,guardCondition:null,unresolved};
  const positive=predicates.length===1?predicates[0]:{op,conditions:predicates},negative=/\bwhile\s+(?:no|not)\b/i.test(raw)||/\b(?:has|have)\s+not\b/i.test(clause);
  return{condition:negative&& !/\bwhile\b/i.test(raw)?{op:'NOT',conditions:[positive]}:positive,guardCondition:negative&&/\bwhile\b/i.test(raw)?{op:'NOT',conditions:[positive]}:null,unresolved:[]};
}
...
export function compileTacticalIntent(text,context={}){
  const raw=String(text||'').trim(),t=norm(raw),initial=norm(raw.split(/\b(?:until|while|when|once|after|unless|only if|not until|release once|release when|commit once|commit when)\b/i)[0]),commands=resolveCommands(raw,context),sideReference=resolveSide(raw,context),terrain=resolveTerrain(raw,context),security=/\b(protect|guard|secure|refuse|cover)\b/.test(initial)&&!(/\b(attack|assault|charge|strike)\b/.test(initial)),order=actionFromText(initial||t,{security}),region=targetRegion(t,{security}),targetCommand=commands.find(c=>c.id!==context.commandId&&(security?(!c.side||c.side===context.ownSide):(!c.side||c.side!==context.ownSide)))||null;
  const {condition:releaseCondition,guardCondition,unresolved:conditionUnresolved}=parseConditionTree(raw,{...context});
  const conditional=/\b(until|while|when|once|after|unless|only if|not until|whichever happens first)\b/.test(t);
