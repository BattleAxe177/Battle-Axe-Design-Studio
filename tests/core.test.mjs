...
test('runtime detector covers major geometry families and deep-scan objects', async()=>{const s=await readFile(new URL('../src/modules/battlefieldDetector.js',import.meta.url),'utf8'); for(const token of ['water','wood','wall','avenue','bridge','structure','track','image,use']) assert.ok(s.includes(token),`missing detector family ${token}`);});
