...
test('deployment restores direct pointer dragging and uses the shared command palette',async()=>{const js=await read('src/modules/deploymentEditor.js'),sides=await read('src/modules/scenarioSides.js');assert.ok(js.includes("el.addEventListener('pointerdown'"));assert.ok(js.includes("dragPiece={kind,id"));assert.ok(js.includes('sideCommandColor(scenario(),faction,index)'));assert.ok(sides.includes("'#164A7A','#2376BD'"));});
test('playtest renders facing vector and unit rotation',async()=>{const js=await read('src/modules/playtestCenter.js');assert.ok(js.includes('class="facing-arrow"'));assert.ok(js.includes('rotate(${facing}deg)'));});
