...
test('deployment restores direct pointer dragging and distinct command palette',async()=>{const js=await read('src/modules/deploymentEditor.js');assert.ok(js.includes("el.addEventListener('pointerdown'"));assert.ok(js.includes("dragPiece={kind,id"));assert.ok(js.includes("#1f5f9f"));assert.ok(js.includes("#3c80c5"));});
...
test('force builder consults accepted scenario rules for camp limits',async()=>{const js=await read('src/modules/scenarioBuilder.js');assert.ok(js.includes('effectiveArmyAssetPolicy'));assert.ok(js.includes("text.includes('two camp')"));assert.ok(js.includes('policy.campMax'));});
