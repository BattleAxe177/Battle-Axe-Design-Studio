...
test('HTML bootstrap imports the module entry point with the VERSION-derived current release cache buster', async()=>{
  const html=await fs.readFile(new URL('../index.html',import.meta.url),'utf8');
  assert.ok(release);
  assert.match(html,new RegExp(`mainEntry='\\./src/main\\.js\\?v=${release.replaceAll('.','\\.')}'`));
