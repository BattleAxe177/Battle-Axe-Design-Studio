...
test('generic command authority follows ancestor/descendant relationships and excludes sibling brigades',()=>{
  const scenario={commands:{French:[
...
test('event-328 regression: a friendly cannon fully masks small-arms LOS and Shoot is not a legal action',()=>{
  const shooter=unit('Union infantry','French',5,7),blocker=unit('Union cannon','French',5,4.6,{profile:'Cannons',traits:['Artillery']}),target=unit('Confederate infantry','Imperial',5,2.2);
...
test('friendly commanders also block LOS',()=>{
  const shooter=unit('Shooter','French',5,7,{width:25,depth:25}),target=unit('Target','Imperial',5,2.8,{width:25,depth:25}),cmd=commander('Friendly commander','French',5,4.9,'other');
...
test('ACW order doctrine uses wider defensive frontage and deeper assault echelons',()=>{
  const makeCtx=order=>{const us=Array.from({length:5},(_,i)=>unit(`r${i+1}`,'French',2+i*.5,8,{commandId:'brig'})),enemy=unit('enemy','Imperial',6,2,{commandId:'enemy'});return{units:[...us,enemy],commanders:[],terrain:[],rules:acwRuleset,scale:1,width:20,height:20,commandParentById:new Map(),tacticalPlan:{commands:{brig:{order,modifiers:{}}}},commandRelease:{}};};
  const defend=makeCtx('Defend');const d=__conformance.activationOrderForSide('French',defend);assert.equal(d.filter(u=>u.maneuverWave===1).length,4);assert.equal(d.filter(u=>u.maneuverWave===2).length,1);
  const assault=makeCtx('Assault');const a=__conformance.activationOrderForSide('French',assault);assert.equal(a.filter(u=>u.maneuverWave===1).length,3);assert.equal(a.filter(u=>u.maneuverWave===2).length,2);
...
test('commander coverage honors command authority instead of proximity to sibling units',()=>{
  const own=unit('own','French',5,5,{commandId:'brig-a'}),sibling=unit('sibling','French',5.3,5,{commandId:'brig-b'}),c=commander('A commander','French',5,5.5,'brig-a'),ctx={units:[own,sibling],commanders:[c],terrain:[],rules:acwRuleset,scale:1,width:12,height:12,commandParentById:new Map([['brig-a','div'],['brig-b','div'],['div','army'],['army',null]])};
...
test('reserve command enters at end of owning turn with commander first and receives no action on entry turn',()=>{
  const st=minimalStudioState();st.project.scenario.commands={French:[{id:'reserve-brig',name:'Reserve Brigade',commander:'Reserve Commander',echelon:'Brigade',commandRating:2,reserve:{enabled:true,deploymentTurn:1,entry:{type:'edge-segment',edge:'north',startPct:40,endPct:60}},units:[{id:'reserve-regt',name:'Reserve Regiment',profile:'Infantry'}]}],Imperial:[{id:'enemy-cmd',name:'Enemy',commander:'',units:[{id:'enemy-regt',name:'Enemy Regiment',profile:'Infantry'}]}]};st.project.scenario.deployment.placements={'enemy-regt':{x:50,y:80,facing:0}};st.project.scenario.structuredRules.turnOneInitiative='French';
...
test('reserve command without commander nominates a temporary command unit only for deployment',()=>{
  const st=minimalStudioState();st.project.scenario.commands={French:[{id:'reserve-brig',name:'No Commander Reserve',commander:'',reserve:{enabled:true,deploymentTurn:1,entry:{type:'point',x:50,y:0}},units:[{id:'u1',name:'First',profile:'Infantry'},{id:'u2',name:'Second',profile:'Infantry'}]}],Imperial:[{id:'enemy-cmd',name:'Enemy',commander:'',units:[{id:'enemy',name:'Enemy',profile:'Infantry'}]}]};st.project.scenario.deployment.placements={enemy:{x:50,y:80,facing:0}};
...
test('structured Turn-1 initiative override changes only the opening side order',()=>{
  const st=minimalStudioState();st.project.scenario.commands={French:[{id:'f',name:'F',commander:'',units:[{id:'fu',name:'French Unit',profile:'Infantry'}]}],Imperial:[{id:'i',name:'I',commander:'',units:[{id:'iu',name:'Imperial Unit',profile:'Infantry'}]}]};st.project.scenario.deployment.placements={fu:{x:20,y:75,facing:0},iu:{x:80,y:25,facing:180}};st.project.scenario.structuredRules.turnOneInitiative='French';const r=runPlaytest(st,{seed:3,turns:1,measurementScale:1});const first=r.events.find(e=>e.side);assert.equal(first?.side,'French');
});
