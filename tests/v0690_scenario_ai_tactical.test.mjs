...
test('patched deterministic engine evaluates canonical ANY reserve predicates against live state',async()=>{
  ...
  assert.equal(result.satisfied,true);assert.equal(result.actor.id,'battery');assert.match(result.reason,/destroyed/i);assert.equal(result.trace.op,'ANY');
  ...
  ctx.terrain=[{id:'line',name:'Named line',parts:[{closed:false,points:[{x:5,y:0},{x:5,y:10}]}]}];ctx.terrainCrossings={'enemy:line:crossed':{actorId:'enemy',actorName:'Enemy',side:'sideA',commandId:'enemy-command',terrainId:'line',terrainName:'Named line',relation:'crossed',turn:2}};ctx.units.push({id:'enemy',name:'Enemy',faction:'sideA',commandId:'enemy-command',destroyed:false,inactive:false});const crossed=engine.__conformance.evaluateCondition690({type:'terrain_crossed',side:'sideA',terrainIds:['line'],terrainName:'Named line'},reserve,ctx);assert.equal(crossed.satisfied,true);assert.equal(crossed.trace.type,'terrain_crossed');
  const checks=[];ctx.turn=3;ctx.commandRelease={};ctx.conditionChecks={};ctx.tacticalPlan={commands:{simmons:{order:'Reserve'}}};ctx.event=(type,actor,payload)=>checks.push({type,payload});const release={op:'ALL',conditions:[{type:'turn_reached',turn:4},{op:'NOT',conditions:[{type:'terrain_crossed',side:'sideA',terrainIds:['other-line'],terrainName:'Other line'}]}]},mods={releaseCondition:release,postReleaseOrder:'Assault'};assert.equal(engine.__conformance.reserveReleased(reserve,ctx,mods,Infinity),false);ctx.turn=4;assert.equal(engine.__conformance.reserveReleased(reserve,ctx,mods,Infinity),true);assert.equal(ctx.commandRelease.simmons.toOrder,'Assault');assert.ok(checks.some(e=>e.type==='tactical_condition_check'&&e.payload.trace.op==='ALL'));assert.ok(checks.some(e=>e.type==='reserve_released'&&e.payload.orderChanged==='Reserve -> Assault'));
});
